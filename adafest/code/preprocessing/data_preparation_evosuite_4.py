...
class PreProcess:
    ...
    def create_understand_database_from_project(cls, root_path=None):
        ...
        for project_ in projects:
            if os.path.exists(root_path + project_+'.udb'):
                print('DB {} already exist.'.format(root_path + project_+'.udb'))
            else:
    ...
    def extract_project_classes_all(cls, udbs_path, class_list_csv_path_root=r'class_list_csvs/'):
        files = [f for f in os.listdir(udbs_path) if os.path.isfile(os.path.join(udbs_path, f)) and f != 'metrics']
        for f in files:
    ...
    def extract_metrics_and_coverage_all(cls, udbs_path: str = r'sf110_without_test',
        ...
        for i, f in enumerate(files):
            ...
            if os.path.exists(csvs_path + f[:-4] + '.csv'):
                continue
            db = understand.open(os.path.join(udbs_path, f))
            ...
            cls.compute_metrics_by_class_list(project_name=f[:-4], database=db, class_list=None, csv_path=csvs_path)
    ...
    def compute_metrics_by_class_list(cls, project_name: str = None, database=None, class_list=None, csv_path=None):
        ...
        class_entities = database.ents('Java Class ~Enum ~Unknown ~Unresolved ~Jar ~Library')
    ...
    def remove_outliers_with_lof(cls, path: str = None, path_new: str = None):
        ...
        for index, row in df2.iterrows():
            ...
            if row['Tests'] < 1:
                ...
                test_effort = ((test_time - 1) / (row['Tests'])) ** (math.ceil(row['Tests'] / row['CSORD_NumberOfMethods']) - 1)
            else:
                test_effort = (1 + ((test_time - 1) / row['Tests'])) ** (math.ceil(row['Tests'] / row['CSORD_NumberOfMethods']) - 1)
