...
class TestabilityMetrics:
    ...
    def compute_java_class_metrics_lexicon(cls, db=None, entity=None):
        ...
        try:
            ...
            print('Error in computing class lexical metrics for class "{0}"'.format(entity.longname()))
...
def do(class_name, database_path):
    import understand
    db = understand.open(database_path)
    class_entity = UnderstandUtility.get_class_entity_by_name(class_name=class_name, db=db)

    one_class_metrics_value = [class_entity.longname()]
    # print('Calculating package metrics')
    package_metrics_dict = TestabilityMetrics.compute_java_package_metrics(db=db, class_name=class_entity.longname())
    if package_metrics_dict is None:
        print('No package metric for item {} was found'.format(class_entity.longname()))
        return

    # print('Calculating class lexicon metrics')
    class_lexicon_metrics_dict = TestabilityMetrics.compute_java_class_metrics_lexicon(db=db, entity=class_entity)
    if class_lexicon_metrics_dict is None:
        print('No class lexicon metric for item {} was found'.format(class_entity.longname()))
        return
    ...
    if class_ordinary_metrics_dict is None:
        print('No class ordinary metric for item {} was found'.format(class_entity.longname()))
        return
...
class PreProcess:
    ...
    def create_understand_database_from_project(cls, root_path=None):
        ...
        projects = [name for name in os.listdir(root_path) if os.path.isdir(os.path.join(root_path, name)) and
                    name!='metrics']
    ...
    def extract_metrics_and_coverage_all(cls, udbs_path: str = r'sf110_without_test',
        ...
        files = [f for f in os.listdir(udbs_path) if
                 os.path.isfile(os.path.join(udbs_path, f)) and os.path.splitext(f)[1] == '.udb']
        ...
        for i, f in enumerate(files):
            ...
            cls.compute_metrics_by_class_list(project_name=f[:-4], database_path=os.path.join(udbs_path, f), class_list=None, csv_path=csvs_path)
    ...
    @classmethod
    def compute_metrics_by_class_list(cls, project_name: str = None, database_path=None, class_list=None, csv_path=None):
        ...
        # class_entities = cls.read_project_classes(project_name=project_name, db=database, df=class_list, )
        # class_entities = database.ents('Java class')
        db = understand.open(database_path)
        class_entities = db.ents('Java Class ~Enum ~Unknown ~Unresolved ~Jar ~Library')
        db.close()
        # for entitiy_ in class_entities:
        #     print(entitiy_.longname(), entitiy_.kind(), entitiy_.parent().longname(), sep='\t\t')

        # print(len(class_entities))
        # quit()
        # res = Parallel(n_jobs=4, )(
        #     delayed(do)(class_entity.longname(), database_path) for class_entity in class_entities
        # )
        with Pool(processes=1) as pool:
            res = pool.starmap(do, zip([class_entity.longname() for class_entity in class_entities],
                                       itertools.repeat(database_path)))
    ...
    def create_complete_dataset(cls, separated_csvs_root: str = r'sf110_csvs_without_test_e3/',
        ...
        for filename in os.listdir(separated_csvs_root):
            try:
                ...
                raise IOError('Failed to read {}'.format(filename))
    ...
    def remove_outliers_with_lof(cls, path: str = None, path_new: str = None):
        ...
        # Add testability for TSE paper
        testability_list = []
        for index, row in df2.iterrows():
            test_quality = (row['Label_LineCoverage'] + row['Label_BranchCoverage'] + row['Label_MutationScore']) / 3.
            test_quality *= 0.01
            test_time = 6.
            if row['Tests'] < 1:
                test_effort = 1.
            elif row['Tests'] <= test_time - 1:
                test_effort = ((test_time - 1) / (row['Tests'])) ** (
                            math.ceil(row['Tests'] / row['CSORD_NumberOfMethods']) - 1)
            else:
                test_effort = (1 + ((test_time - 1) / row['Tests'])) ** (
                            math.ceil(row['Tests'] / row['CSORD_NumberOfMethods']) - 1)
            t_X = test_quality / test_effort
            if t_X > 1.:
                print(row['Tests'], test_quality, row['CSORD_NumberOfMethods'], test_effort)
                print('ERROR')
            testability_list.append(t_X)

        df2['Testability'] = testability_list
        df2.drop(columns=['Label_LineCoverage', 'Label_BranchCoverage', 'Label_MutationScore', 'Tests'], inplace=True)
        print(df2)
        print('number of outliers', len(df.index) - len(df2.index))
        df2.to_csv(path_new, index=False)
        quit()

        df.drop(columns=['Label_Combine1', 'Output', 'Exceptions', 'Label_Combine2', 'Label_MaxLineAndBranch',
                         'Label_MinLineAndBranch'], inplace=True)
